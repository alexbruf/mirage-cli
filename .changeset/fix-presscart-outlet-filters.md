...
With a filter active, `--limit` becomes the page size over matches and `--page` selects the slice; `--page` without `--limit` is refused rather than silently answered with page 1.
