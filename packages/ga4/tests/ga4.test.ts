...
import { buildProgram, ga4Command } from "../src/index.ts";

const dec = new TextDecoder();

describe("@mirage-cli/ga4", () => {
  test("buildProgram() returns the ga4 Command", () => {
  ...
  test("runCommander(program, ['--help']) → exitCode 0 + usage on stdout", async () => {
    ...
    expect(dec.decode(r.stdout)).toContain("Usage: ga4");
  });

  test("ga4Command (MirageCommandFn) routes argv via texts and produces stdout stream", async () => {
    const [stdout, ioresult] = await ga4Command(null, [], ["--version"], {
      stdin: null,
      flags: {},
    });
