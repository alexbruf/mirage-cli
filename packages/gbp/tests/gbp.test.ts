...
import { buildProgram, gbpCommand } from "../src/index.ts";

const dec = new TextDecoder();

describe("@mirage-cli/gbp", () => {
  test("buildProgram() returns the gbp Command", () => {
  ...
  test("runCommander(program, ['--help']) → exitCode 0 + usage on stdout", async () => {
    ...
    expect(dec.decode(r.stdout)).toContain("Usage: gbp");
  });

  test("gbpCommand (MirageCommandFn) routes argv via texts and produces stdout stream", async () => {
    const [stdout, ioresult] = await gbpCommand(null, [], ["--version"], {
      stdin: null,
      flags: {},
    });
