...
export function renderList(envelope: unknown, records: unknown[], format: Format): string {
  switch (format) {
    ...
    case "csv":
      return toCsv(records.map(flattenRecord));
    case "table":
      return toTable(records.map(flattenRecord));
  }
}
...
export function renderObject(obj: unknown, format: Format): string {
  switch (format) {
    ...
    case "csv":
      return toCsv([flattenRecord(obj)]);
    case "table": {
      if (obj && typeof obj === "object" && !Array.isArray(obj)) {
        const flat = flattenRecord(obj) as Record<string, unknown>;
        const rows = Object.entries(flat).map(([field, value]) => ({
          field,
...
/**
 * HubSpot CRM records nest their fields under `properties` — lift those up one
 * level for table/csv so `firstname` lands as a column instead of a blob.
 */
function flattenRecord(row: unknown): unknown {
  if (!row || typeof row !== "object" || Array.isArray(row)) return row;
  const obj = row as Record<string, unknown>;
  const props = obj.properties;
  if (props && typeof props === "object" && !Array.isArray(props)) {
    const { properties: _omit, ...rest } = obj;
    return { ...rest, ...(props as Record<string, unknown>) };
  }
  return obj;
}
