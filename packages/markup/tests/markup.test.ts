...
import { buildProgram, markupCommand } from "../src/index.ts";

const dec = new TextDecoder();

describe("@mirage-cli/markup", () => {
  test("buildProgram() returns the markup Command", () => {
  ...
  test("runCommander(program, ['--help']) → exitCode 0 + usage on stdout", async () => {
    ...
    expect(dec.decode(r.stdout)).toContain("Usage: markup");
  });

  test("markupCommand (MirageCommandFn) routes argv via texts and produces stdout stream", async () => {
    const [stdout, ioresult] = await markupCommand(null, [], ["--version"], {
      stdin: null,
      flags: {},
    });
