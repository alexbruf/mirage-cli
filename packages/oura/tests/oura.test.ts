...
import { buildProgram, ouraCommand } from "../src/index.ts";

const dec = new TextDecoder();

describe("@mirage-cli/oura", () => {
  test("buildProgram() returns the oura Command", () => {
  ...
  test("runCommander(program, ['--help']) → exitCode 0 + usage on stdout", async () => {
    ...
    expect(dec.decode(r.stdout)).toContain("Usage: oura");
  });

  test("ouraCommand (MirageCommandFn) routes argv via texts and produces stdout stream", async () => {
    const [stdout, ioresult] = await ouraCommand(null, [], ["--version"], {
      stdin: null,
      flags: {},
    });
