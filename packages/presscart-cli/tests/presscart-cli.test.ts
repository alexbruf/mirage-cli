...
describe("@mirage-cli/presscart-cli", () => {
  ...
  test("outlets list and products listings expose --min-price/--max-price", () => {
...
describe("price helpers (unit_amount is whole USD, not cents)", () => {
  const appleNews = { name: "Apple News", prices: [{ unit_amount: 775, pricing_tier: "basic" }] };
  const cheap = { name: "Blog", prices: [{ unit_amount: 175 }, { unit_amount: 225 }] };
  const noPrice = { name: "Mystery", prices: [] };

  test("rowPriceUsd returns the lowest unit_amount as dollars", () => {
  ...
  test("filterByPrice respects min/max bounds and excludes priceless rows when bounded", () => {
    ...
    expect(filterByPrice(rows, {})).toHaveLength(3); // no bounds: passthrough
