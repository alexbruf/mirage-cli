...
const CONFIG_DIR = join(homedir(), ".config", "pulse");
const SESSION_PATH = join(CONFIG_DIR, "session.json");

const DEFAULT_BASE_URL = "https://pulse.viewengine.ai";

export function getDefaultBaseUrl(): string {
  return process.env.PULSE_API_BASE_URL ?? DEFAULT_BASE_URL;
}
...
export function loadSession(): Session | null {
  const envKey = process.env.PULSE_API_KEY;
  if (envKey) {
    return {
      apiKey: envKey,
  ...
  const envOAuth = process.env.PULSE_OAUTH_ACCESS_TOKEN;
  if (envOAuth) {
    return {
...
export function requireSession(): Session {
  const s = loadSession();
  if (!s) {
    console.error("Not logged in. Run: pulse login  (or: pulse login --api-key <key>)");
    process.exit(1);
  }
  return s;
}
