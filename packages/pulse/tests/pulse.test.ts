...
import { buildProgram, pulseCommand } from "../src/index.ts";

const dec = new TextDecoder();

describe("@mirage-cli/pulse", () => {
  test("buildProgram() returns the pulse Command", () => {
    const program = buildProgram();
    expect(program.name()).toBe("pulse");
  ...
  test("runCommander(program, ['--help']) → exitCode 0 + usage on stdout", async () => {
    ...
    expect(dec.decode(r.stdout)).toContain("Usage: pulse");
  });

  test("pulseCommand (MirageCommandFn) routes argv via texts and produces stdout stream", async () => {
    const [stdout, ioresult] = await pulseCommand(null, [], ["--version"], {
      stdin: null,
      flags: {},
    });
